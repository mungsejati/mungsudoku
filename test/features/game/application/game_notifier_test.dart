...
void main() {
  ...
  test('Test 1: initNewGame sets isLoading to true then false', () async {
    ...
    expect(
      ...
      isTrue,
      reason: 'Should have emitted isLoading: true',
    );
    expect(
      states.last.isLoading,
      isFalse,
      reason: 'Final state should have isLoading: false',
    );
  });
  ...
  test('Test 2: local save is cleared when isVictory is true', () async {
    ...
    expect(
      prefs.containsKey('current_game'),
      isTrue,
      reason: 'Save should exist after moves',
    );
    ...
    expect(
      prefs.containsKey('current_game'),
      isFalse,
      reason: 'Save should be cleared on victory',
    );
  });
  ...
  test('Test 3: wrong input triggers conflict', () async {
    ...
    final cells = List.generate(
      ...
      (r) => List.generate(
        size,
        (c) => SudokuCell(row: r, col: c, solutionValue: 1, value: 0),
      ),
    );
    cells[0][0] = const SudokuCell(
      row: 0,
      col: 0,
      value: 1,
      solutionValue: 1,
    ...
    expect(
      notifier.state.conflictPositions.isNotEmpty,
      isTrue,
      reason: 'Should flag conflict',
    );
    expect(
      notifier.state.cumulativeMistakeCount,
      equals(1),
      reason: 'Mistake count should increase',
    );
  });
  ...
  test('Test 4: Auto-Complete sets isAutoCompleteRunning to true', () async {
    ...
    for (int r = 0; r < notifier.state.board.gridSize && emptyCount > 9; r++) {
      for (
        int c = 0;
        c < notifier.state.board.gridSize && emptyCount > 9;
        c++
      ) {
        if (!notifier.state.board.cellAt(r, c).isFilled) {
          notifier.inputNumber(
            r,
            c,
            notifier.state.board.cellAt(r, c).solutionValue,
          );
          emptyCount--;
        }
      }
  ...
  test(
    'Test 5: restartPuzzle restores initial board and resets mistakes',
    () async {
      final container = makeProviderContainer();
      final notifier = container.read(gameNotifierProvider.notifier);
      ...
      expect(
        notifier.state.board.filledCellCount,
        greaterThan(initialBoard.filledCellCount),
      );
      ...
      expect(
        notifier.state.board.filledCellCount,
        equals(initialBoard.filledCellCount),
      );
  ...
  test('Test 6: useHint auto-prunes notes from peers', () async {
    ...
    expect(
      filledCell.notes,
      isEmpty,
      reason: 'Hint should clear notes on target cell',
    );
    ...
    expect(
      peerCell.notes,
      isNot(contains(correctVal)),
      reason: 'Hint should auto-prune notes from peers',
    );
  });
  ...
  test('Test 7: invalid note triggers conflict and is not added', () async {
    ...
    expect(
      notifier.state.board.cellAt(0, 1).notes.contains(1),
      isFalse,
      reason: 'Invalid note should not be added',
    );

    // It should flag (0,0) as conflict
    expect(
      notifier.state.conflictPositions,
      contains(const (0, 0)),
      reason: 'Should flag conflicting peer position',
    );
  });
}
